class Jeu():
    ...
    def ajoutJeu(self, nom_du_jeu, editeur, annee_de_sortie):
        # On essaie de caster les données saisies, 
        # si erreur de cast on renvoie un message
        try : 
            nom_du_jeu = str(nom_du_jeu)
            editeur = str(editeur)
            annee_de_sortie = int(annee_de_sortie)
        except:
            return 'Les données saisies ne sont pas correctes'
        
        # On vérifie si le jeu existe déjà, sinon on lève une exception
        if self.__trouverunjeu(nom_du_jeu) :
            raise Exception('Le jeu existe déjà dans la liste')
        jeu = '\n' + nom_du_jeu + ',' + editeur + ',' + str(annee_de_sortie)
